...
const TYPE_LABEL = { useful: "полезное", ok: "допустимое", wasteful: "бесполезное" };
const TYPE_COLOR = { useful: "#5CE8A4", ok: "#E8C547", wasteful: "#F97B5C" };
        ...
        {view==="tracker" && (
          <div>
            ...
            {(() => {
              ...
              const untrackedMs = Math.max(0, msSinceStart - todayTrackedMs);
              return (
                <div style={{marginTop:16,background:"#12122a",border:"1px solid #1e1e3a",borderRadius:14,padding:18}}>
            ...
            {liveTotal>0 && (
              <div style={{marginTop:12,background:"#12122a",border:"1px solid #1e1e3a",borderRadius:14,padding:18}}>
          ...
          <div>
            ...
            {grandTotal > 0 && (() => {
              ...
              return (
                <div style={{background:"#12122a",border:"1px solid #1e1e3a",borderRadius:14,padding:18,marginBottom:12}}>
                  <div style={{fontSize:10,color:"#404060",letterSpacing:".15em",marginBottom:12}}>ОТЧЁТ ЗА ПЕРИОД</div>
                  <div style={{fontSize:13,color:"#c0c0e0",lineHeight:1.8,marginBottom:14}}>
                    Ты потратил{" "}
                    <span style={{color:TYPE_COLOR.useful,fontWeight:600}}>{usefulPct}% на полезное</span>,{" "}
                  ...
                  {[["useful",usefulPct],["ok",okPct],["wasteful",wastePct]].map(([t,pct])=>(
                    <div key={t} style={{marginBottom:8}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#606080",marginBottom:4}}>
                        <span>{TYPE_LABEL[t]}</span><span>{pct}%</span>
                      </div>
                      <div style={{height:4,background:"#1a1a3a",borderRadius:2,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${pct}%`,background:TYPE_COLOR[t],borderRadius:2,transition:"width .6s"}}/>
                      </div>
                    </div>
                  ))}
                  {/* efficiency score */}
                  <div style={{marginTop:16,padding:"12px 14px",background:"#0d0d1a",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:"#606080"}}>полезное +1 · допустимое 0 · бесполезное −1</div>
                    </div>
                    <div style={{fontSize:28,fontFamily:"'Syne',sans-serif",fontWeight:800,color:scoreColor,minWidth:60,textAlign:"right"}}>
            ...
            {(() => {
              ...
              for (let i = 6; i >= 0; i--) {
                ...
                const f = d.getTime(), t = f + 86400000;
                const dt = aggregateSessions(sessions, f, Math.min(t, Date.now()));
                const totalMin = Object.values(dt).reduce((a,b)=>a+b,0) / 60000;
                let score = 0;
                if (totalMin > 0) {
                  let s = 0;
                  CATEGORIES.forEach(c => { s += (dt[c.id]||0)/60000 * TYPE_SCORE[c.type]; });
                  score = s / totalMin;
                }
                const label = i === 0 ? "Сег" : ["Вс","Пн","Вт","Ср","Чт","Пт","Сб"][d.getDay()];
                days.push({ label, score, hasData: totalMin > 0 });
              }
              const hasAny = days.some(d => d.hasData);
                ...
                <div style={{background:"#12122a",border:"1px solid #1e1e3a",borderRadius:14,padding:18,marginBottom:12}}>
                  ...
                  <svg viewBox={`-4 0 108 ${H+20}`} style={{width:"100%",overflow:"visible"}}>
                    ...
                    {days.map((d,i) => {
                      const x = (i/6)*100;
                      const y = H - ((d.score-minS)/range)*(H-10) - 5;
                      const c = d.score > 0.15 ? "#5CE8A4" : d.score < -0.15 ? "#F97B5C" : "#E8C547";
                      return d.hasData ? (
                        <g key={i}>
                          <circle cx={x} cy={y} r="3" fill={c}/>
                          <text x={x} y={H+14} textAnchor="middle" fontSize="7" fill="#505070">{d.label}</text>
                          <text x={x} y={y-6} textAnchor="middle" fontSize="6.5" fill={c}>
                            {d.score>0?"+":""}{d.score.toFixed(1)}
                          </text>
                        </g>
                      ) : (
                        <text key={i} x={x} y={H+14} textAnchor="middle" fontSize="7" fill="#303050">{d.label}</text>
                      );
                    })}
                  </svg>
                </div>
                        ...
                        return (
                          <div key={cat.id}>
                            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                              ...
                              <span style={{fontSize:10,color:TYPE_COLOR[cat.type],marginRight:4}}>{TYPE_LABEL[cat.type]}</span>
