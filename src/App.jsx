        ...
        {view==="tracker" && (
          <div>
            ...
            {(() => {
              ...
              const msSinceStart = Date.now() - todayStart.getTime();
              const trackedMs = liveTotal;
              const trackedH = (trackedMs / 3600000).toFixed(1);
              const totalH = (msSinceStart / 3600000).toFixed(1);
              const pct = Math.min(100, (trackedMs / msSinceStart) * 100);
              const untrackedH = Math.max(0, msSinceStart - trackedMs);
              return (
                <div style={{marginTop:16,background:"#12122a",border:"1px solid #1e1e3a",borderRadius:14,padding:18}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#505070"}}>
                    ...
                    <span>❓ не зафиксировано {formatDuration(untrackedH*3600000,true)||"0с"}</span>
                  </div>
                </div>
              );
            })()}
